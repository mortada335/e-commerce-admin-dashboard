...
export default function usePatch ({ queryKey, ...options })  {
  const queryClient = useQueryClient()
  const { toast } = useToast()
 
  // Function to perform a patch request to the specified endpoint with the given body
  const patchEndpoint = async (endpoint, body, id, headers) => {
    return axiosPrivate.patch(`${endpoint}${id}/`, body, { headers });
  };
  ...
  return useMutation({
    ...
    mutationFn: ({ endpoint, body, id, headers }) =>
      options.mutationFn // If a custom mutation function is provided, use it
        ? options.mutationFn()
        : patchEndpoint(endpoint, body, id, headers), // Otherwise, use the default patchEndpoint function
    onError: (...params) =>
