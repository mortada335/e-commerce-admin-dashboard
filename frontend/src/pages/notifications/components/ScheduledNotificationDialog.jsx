...
import { SCHEDULED_NOTIFICATION_URL, SEND_NOTIFICAtION_URL, USERS_URL } from "@/utils/constants/urls";
...
import {  customFormatDate, formatDateToISO,  formatFullDate } from "@/utils/methods";
...
import { setIsScheduledNotificationDialogOpen, useNotificationStore } from "../store";
...
export default function ScheduledNotificationDialog() {
    const queryClient = useQueryClient()
    const {t} = useTranslation()
    const { isScheduledNotificationDialogOpen, selectedNotification } =
  ...
  const onFinish = async () => {
    ...
    form.reset();
    setIsScheduledNotificationDialogOpen(false);
    await queryClient.invalidateQueries({ queryKey: ['SentNotification'] }); // Invalidate queries to refresh data
  };
  ...
  const {
    ...
    queryKey: "ScheduledNotification",
    onSuccess: onFinish,
  });


    useEffect(() => {
    if (selectedNotification?.id && isScheduledNotificationDialogOpen) {
      console.log(selectedNotification);
      
      setFormFields({
        title: selectedNotification?.title,
        body: selectedNotification?.body,
        send_date: selectedNotification?.send_at,
        url: selectedNotification?.data?.url,

      
       
      });

      form.setValue("title", selectedNotification?.title);
      form.setValue("body", selectedNotification?.body);
      form.setValue("send_date", selectedNotification?.send_at);
      form.setValue("url", selectedNotification?.data?.url);
      if (selectedNotification?.data?.manufacturer_id) {
        
        form.setValue("filter_id", selectedNotification?.data?.manufacturer_id);
      }
      if (selectedNotification?.data?.category_id) {
        
        form.setValue("filter_id", selectedNotification?.data?.category_id);
      }
      if (selectedNotification?.data?.product_id) {
        
        form.setValue("filter_id", selectedNotification?.data?.product_id);
      }
  ...
  const onSubmit = async () => {
    ...
    if (!formFields.title || !formFields.body) {
     ...
     const payloadData={}
    if (formFields.url)
      payloadData.url=formFields.url;
  
    if (formFields.type === "category" && formFields.filter_id!==null )
      payloadData.category_id=formFields.filter_id;
    if (formFields.type === "product" && formFields.filter_id!==null )
      payloadData.product_id=formFields.filter_id;
    
    // if (selectedCustomers?.length) {

    //   payloadData.customer_ids= selectedCustomers?.map((customer)=>customer?.id);
    // }

    if (formFields.url ||  formFields.filter_id!==null ) {
      payload.data=payloadData
    }
  
    //  putMutate({
    //     endpoint: SCHEDULED_NOTIFICATION_URL,
  ...
  return (
    <Dialog open={isScheduledNotificationDialogOpen} onOpenChange={setIsScheduledNotificationDialogOpen}>
      <DialogContent className="w-[95%] sm:max-w-[750px] px-4 ">
      <ScrollArea className=" w-[99%] md:w-full border-none">

        <ScrollArea className=" h-[400px] md:h-[550px] lg:h-[650px] xl:h-fit  pr-4 w-full ">
          <DialogHeader className="rtl:items-end">
            <DialogTitle>{t("Scheduled Notification")}</DialogTitle>
            <DialogDescription>
              {t("Scheduled Notification here. Click save when you are done.")}
            </DialogDescription>
          </DialogHeader>
                  ...
                  <FormItem className="space-y-3 px-1">
                    ...
                    <FormControl>
                      <RadioGroup
                        ...
                        className="flex  space-x-1 rtl:flex-row-reverse"
                      >
