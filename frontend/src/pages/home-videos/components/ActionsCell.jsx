...
const ActionsCell = ({ item }) => {
  ...
  // Edit HomeVideo Action
  const handleEditHomeVideo = () => {
    setSelectedHomeVideo(item);
    setIsHomeVideoDialogOpen(true);
  };
  // Delete HomeVideo Action
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuSeparator />
          <Can permissions={["app_api.change_shorthomepagevideo"]}>
            <DropdownMenuItem
              className="flex justify-start items-center space-x-2 cursor-pointer"
              onClick={handleEditHomeVideo}
            >
              <SquarePen size={16} /> <span>{t("Edit")}</span>{" "}
            </DropdownMenuItem>
          </Can>
          <Can permissions={["app_api.delete_shorthomepagevideo"]}>
            <DropdownMenuItem
