...
const RewardCoupons = () => {
  ...
  if (filters?.length) {
    filters.forEach((filter) => {
      ...
      if (filter.type === "text" || filter.type === "search") {
        if (valueToSend) params.append(searchBy || "code", valueToSend);
        return;
      }
  ...
  params.append("exclude_points_coupon", false);
  ...
  const totalPages = Math.ceil(RewardCoupons?.data?.count / itemPerPage); // Assuming 25 items per page
  ...
  const exportCsvHandler = async () => {
    ...
    //       : "-date_added",
    //     columns:
    //       "coupon_id,name,code,type,discount,total_min,total_max,uses_total,uses_customer,for_customer_id,date_added,date_start,date_end",
    //     // eslint-disable-next-line no-unused-vars
    //   }).filter(([_, value]) => value !== undefined && value !== null)
    // );
    ...
    try {
      ...
      //   setCsvDownload(false);
      //   downloadCsv(response.data, undefined, "Promo Codes.csv");
      // }

      if (RewardCoupons.data?.results?.length) {

        const currentData = RewardCoupons.data?.results?.map((coupon)=> { 
        ...
        // Reset the state and initiate the CSV file download.
        setCsvDownload(false);
        exportToExcel(currentData, "Reward Promo Codes.xlsx");
      ...
      <Section className="space-y-8 h-fit items-start">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>{t("Reward Promo Codes")}</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        <HeaderText className={"w-full text-start "} text={t("Reward Promo Codes")} />
        {/* {isFilterMenu && (
          <Card className="w-full">
            ...
            <CardContent className="flex items-center justify-start flex-wrap w-full gap-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectLabel>{t("Reward Promo Codes Status")}</SelectLabel>
        ...
        <WrapperComponent
          isEmpty={!RewardCoupons?.data?.results?.length > 0}
          isError={isError}
          error={error}
          isLoading={isLoading}
        ...
        >
          <DataTable
            columns={columns}
            data={RewardCoupons?.data?.results?.map((coupon) => ({
              id: coupon.coupon_id,
              name: coupon.name,
              code: coupon.code,
              type: coupon.type,
              ...
              date_added: coupon.date_added
                ? formatFullDate(coupon.date_added)
                : "",
              date_start: coupon.date_start
                ? formatFullDate(coupon.date_start)
                : "",
              date_end: coupon.date_end ? formatFullDate(coupon.date_end) : "",
          ...
          <Card className="flex items-center justify-center space-x-2 py-2 px-0 w-full">
            <Pagination
              ...
              next={RewardCoupons?.data?.next}
              previous={RewardCoupons?.data?.previous}
              totalPages={totalPages}
              totalCount={RewardCoupons?.data?.count}
              page={page}
              setPage={setPage}
            />
          </Card>
